...
IF exists (SELECT NULL
            ...
            WHERE accountno = @OldAccountNoClean AND carrier_key = 55)
BEGIN
...
IF exists (SELECT NULL
            FROM AuditItemCarrierModeClientSettingsTBL
            WHERE accountno = @NewAccountNoClean AND carrier_key = 55)
BEGIN
...
IF exists (SELECT NULL
            FROM dbo.UPSEBillDownloadSettingsTBL
            WHERE client_key = @client_key
            AND clientloc_key = @clientloc_key
            AND AccountNo = @NewAccountNo)
BEGIN
    Print 'ERROR!! UPSEBillDownloadSettingsTBL record already Exists for new account no: ' + @NewAccountNo
    SELECT * 
    FROM dbo.UPSEBillDownloadSettingsTBL
    WHERE client_key = @client_key
        AND clientloc_key = @clientloc_key
